...
module.exports.getAnnouncements = async (
    parent, 
    args, 
    { redisClient, req }
) => {
    if (announcementRoles.includes(req.role)){
        try {
            ...
            if (!hasKeys && cached) return JSON.parse(cached, dateReviver);
            if (!hasKeys && !cached) {
...
module.exports.createAnnouncement = async (
    parent, 
    args, 
    { redisClient, req }
) => {
...
module.exports.updateAnnouncement = async (
    parent, 
    args, 
    {redisClient, req}
 ) => { 
    if (announcementRoles.includes(req.role)) {
        try {
            const queryArgs = getQueryArgs(args);
            const currentDate = Date.now();
            if (queryArgs) queryArgs.updated_at = currentDate;
            const { _id } = queryArgs;
...
module.exports.deleteAnnouncement = async (
    parent, 
    args, 
    { redisClient, req}
) => {
